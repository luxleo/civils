...
//TODO: make DistributedLoad as TrapezoidalLoad so that it can be applied as general distributed load
export class DistributedLoad implements Load {
    private readonly startPosition: number;
    private readonly endPosition: number;
    private readonly magnitude: number; // 단위 길이당 하중 크기
    
    constructor(startPosition: number, endPosition: number, magnitude: number) {
    ...
    // 전체 하중의 합 (크기 x 길이)
    getMagnitude(): number {
        return this.magnitude * (this.endPosition - this.startPosition);
    }
...
export class TriangularDistributedLoad implements Load {
    private readonly startPosition: number;
    private readonly endPosition: number;
    private readonly startMagnitude: number; // 시작점에서의 단위 길이당 하중 크기
    private readonly endMagnitude: number; // 끝점에서의 단위 길이당 하중 크기
    
    constructor(startPosition: number, endPosition: number, startMagnitude: number, endMagnitude: number) {
        if (startPosition < 0) throw new Error("시작 위치는 양수로 해주세요");
        if (endPosition <= startPosition) throw new Error("끝 위치는 시작 위치보다 커야 합니다");
    ...
    getPosition(): number {
        ...
        if (this.startMagnitude === 0) {
            ...
            return this.startPosition + (this.endPosition - this.startPosition) * 1/3;
        } else {
            // 사다리꼴 형태
    ...
    isTriangular(): boolean {
        return this.startMagnitude === 0 || this.endMagnitude === 0 || 
               this.startMagnitude !== this.endMagnitude;
    }
    
    // DistributedLoad 인터페이스와의 호환성을 위한 메서드
    getUnitMagnitude(): number {
        // 평균 단위 하중 반환
